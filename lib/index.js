...
const serverUrl = 'https://dev1.ssd.oscp.cloudpose.io:7000';
...
export function requestServices(countryCode, h3Index) {
    ...
    return request(`${serverUrl}/${countryCode}/ssrs?h3Index=${h3Index}`);
}

export function requestService(countryCode, id) {
    if (local) {
        return localService;
    }
    ...
    return request(`${serverUrl}/${countryCode}/ssrs/${id}`);
}

function request(url) {
    return fetch(url, {
            headers: {
                Accept : 'application/vnd.oscp+json; version=1.0;'
            }
        })
        .then(response => {
            if (response.ok) {
...
export async function getSchema() {
    return await fetch('./lib/ssr.schema.json')
        .then( response => response.json())
        .then( data => data)
        .catch(error => {
            throw new Error(`Error fetching ssr schema: ${error}`);
        })
}
